...
The contract lets builders submit a claim, attach an evidence URL, commit a bond amount, and resolve the claim through GenLayer consensus. Validators independently read the same evidence and classify the claim as `supported`, `contradicted`, `unverifiable`, or `stale`.
...
- The contract stores the claim, evidence URL, bond, status, verdict, confidence bucket, and material facts.
...
`submitted`
: A claim has been registered with claim text, evidence URL, and a committed bond amount.

`resolved`
: Consensus produced a verdict, confidence bucket, summary, and material facts.

`challenged`
: The submitter or owner has supplied replacement evidence and requested another review.
...
4. Requires material facts to overlap enough to show both outputs relied on the same evidence.
...
`submit_claim(claim_id, claim_text, evidence_url, bond_atto, submitted_at)`
: Registers a new claim. The claim text must be meaningful, the URL must be HTTP(S), the bond must meet the minimum, and IDs cannot be reused.
...
The local test checks the pinned runner header, forbidden runner aliases, expected public methods, helper functions, verdict constants, confidence constants, and `run_nondet_unsafe` usage.
